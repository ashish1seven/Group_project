...
const Creategroup = () => {
  return (
    <>
      <div className=" h-[85vh] text-white px-52 py-44 text-center" style={{backgroundImage:"url('https://modulescomposer.s3.us-east-2.amazonaws.com/eventpro/header.jpg')",
      backgroundPosition: "center",
      backgroundSize: "cover",
      backgroundAttachment: "fixed",
    }}>
        <img src={logo} alt="" className="h-16 mb-10 m-auto" />
        <h1 className="font-bold text-4xl mb-4">Connect: The App That Links Hearts and Minds</h1>
        ...
        <Link
          ...
          to={"/start"} target='_blank'
        >
          <button>Start a group</button>
        </Link>
      </div>
      <section className="w-screen mb-10">
        <h1 className="font-bold text-4xl text-center my-8"><span className="text-[#FF0079]">Connect</span>   makes it easy to build a community</h1>
        <div className="flex items-center justify-evenly ">
        {
          benefits.map(item=>{
            return(
             
                <div key={item.id} className='flex flex-col items-center w-[220px] h-[200px] justify-evenly'>
                <img src={item.logo} alt="" className="h-10 rounded-lg"/>
                <h1 className=" text-xl text-center font-bold ">{item.title}</h1>
                <p className="text-center text-sm"> {item.description} </p>
                <Link to={'/start'} target='_blank' className="text-[#FF0079] font-semibold text-sm"><button>START NOW</button></Link>
              </div>
      ...
      <section className="bg-[#F5DEB3]/80 p-10">
        <h1 className="font-bold text-4xl text-center my-3">Make 2023 your year with <span className="text-[#FF0079]">Connect!</span>  </h1>
        <p className="text-center text-sm mb-12">Quickly make new friends with over 60 million users, <br /> all here to connect over shared interests, build professional networks, and have a blast.</p>
            ...
            features.map((item)=>{
              return(
                <Link key={item.id}>
                  ...
                  <div className="h-[250px] w-full" style={{backgroundImage:`url(${item.logo})`,backgroundSize:'cover'}}></div>
                  <div className="flex flex-col items-start justify-evenly p-4">
                    <h1 className="text-[#FF0079] font-bold text-lg">{item.title}</h1>
      ...
      <section className="h-[250px] flex justify-evenly p-10">
        <div className="w-[50%]">
        <h1 className="text-2xl font-bold mb-3 ">Find Your People in 2023.</h1>
        <p className="text-lg text-left mb-10">Make time for your interests and connect with like-minded individuals. Lasting connections are made on Connect.</p>
        <Link
          className="border-2 text-white border-[#FF0079] mr-5 w-[150px] text-center p-2 rounded-md font-bold bg-[#FF0079] hover:bg-[#FF0079]/80"
          to={"/start"} target='_blank'
        >
